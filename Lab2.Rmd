...
## Pull NY county map with county names
...
# define map ids, goes to all 62 strings (pairs of names) and pull 2nd name only (county names)
map.IDs = as.character(tolower(sapply(list.names.ny, function(x) x[2])))
head(map.IDs, n = 62)
map.IDs = gsub(" ", "",  map.IDs)
head(map.IDs, n = 62)

# Create spatial polygon
require(maptools)
ny_cty_sp = map2SpatialPolygons(ny_cty, IDs = map.IDs, 
...
# fread is like read_csv but able to read huge amounts of information
...
ny_rwj$County = gsub(" ", "", ny_rwj$County)
...
#transforming quality of life to numeric
ny_rwj_df$QL.Rank = as.numeric(ny_rwj_df$QL.Rank)
summary(ny_rwj_df$QL.Rank)

# reverse coding quality of life variable 
...
# create shape file names rwj_ny and save on computer
require(rgdal)
writeOGR(ny_rwj_df, 
...
# read shape file from working directory into r
require(sf)
...
# create string of coordinates from file and plot
